...
ptf_mv_returns.name = 'MV Portfolio'

## Compute Results
perf_ptf_mv = perf(ptf_mv_returns, libor_US_mean)
...
perf_ptf_merged = pd.concat([perf_ptf_mdp, perf_ptf_sr, perf_ptf_mv, perf_ptf_ew, ], axis=1)
perf_ptf_merged.to_latex('Output/perf_ptf_merged.tex', column_format = 'lccfc', multicolumn_format='c')

plt.figure(figsize=(10,7))
plt.plot(cum_prod(ptf_mdp_returns), 'r', label = 'MDP')
plt.plot(cum_prod(ptf_sr_returns), 'b', label = 'SR')
plt.plot(cum_prod(ptf_ew_returns), 'g', label = 'EW')
plt.plot(cum_prod(ptf_mv_returns),'c' , label='Min Vol')
plt.legend(loc='upper left', frameon=True)
plt.title("Cumulative Return", fontsize=15)
...
weights_ptf_merged = pd.DataFrame({'MDP': weights_assets_mdp[start_ptf:].mean(), 
                                   'SR': weights_assets_sr[start_ptf:].mean(), 
                                   'Min Vol': weights_assets_mv[start_ptf:].mean()})
weights_ptf_merged['EW'] = [1/3, 1/3, 1/3]
weights_ptf_merged.round(3).to_latex('Output/weights_merged.tex', column_format = 'lcfcc', multicolumn_format='c')
